classdef SimulationClass < handle
  ...
  methods
    ...
    function [] = generateStateFunction(obj)
      %% GENERATESTATEFUNCTION Generates a function that calculates all the
      % states of the model.
      %

      name = [class(obj.model) 'State'];
      
      tokenReduced = obj.model.isReduced;
      obj.model.isReduced = true;

      % Open driver file.
      f = fopen(['./build/' name '.m'],'w');

      % Function definition.
      fprintf(f,'function [out] =  %s(t,x,p)\n',name);
      fprintf(f,'%% This script was autogenerated with OneModel %s.\n\n',OneModel.version()); 
      fprintf(f,"out.t = t;\n\n");
      ...
      for i = 1:obj.model.varsNum
        fprintf(f,'out.%s = x(:,%d);\n',char(obj.model.vars(i)),i);
      end
      fprintf(f,'\n');
      
      % Write substitution variables.
      obj.model.isReduced = false;
      subsVars = obj.model.vars(obj.model.isSubs);
      obj.model.isReduced = true;

      sf = char(obj.fncSubsModel);
      % Remove the header.
      sf = extractAfter(sf,min(strfind(sf,'[')));
      % Remove the end bracket.
      sf = extractBefore(sf,min(strfind(sf,']')));
    ...
    end % generateStateFunction

    function [p,x0,M] = generateParamFunction(obj,name)
      %% GENERATEPARAMFUNCTION Generates a function which returns the 
      % default parameters for simulating the model.
      %
      % param: name [char] Name for the generated function.
      %
      % return: p  Struct with default parameter values.
     	%       : x0 Array with default initial conditions values.
     	%       : M  Mass matrix for DAE simulations.

      if nargin < 2
        name = [class(obj.model) 'DefaultParam'];
      end
      ...
      % Function definition.
      fprintf(f,'function [p,x0,M] =  %s()\n',name);

      fprintf(f,'%% This script was autogenerated with OneModel %s.\n\n',OneModel.version());
      
      % Parameters definition.
      fprintf(f,'%% Default parameters value.\n');
      params = obj.model.params;
      for i = 1:length(params)
        fprintf(f,'p.%s = %e;\n',char(params(i)),obj.model.paramsValue(i));
      end
      fprintf(f,'\n');

      % Initial conditions.
      fprintf(f,'%% Default initial condition value.\n');
      fprintf(f,'x0 = [\n');
      for i = 1:obj.model.varsNum
        fprintf(f,'\t %e %% %s\n',obj.model.varsStart(i),char(obj.model.vars(i)));
      end
      fprintf(f,'];\n\n');

      % Mass matrix.
      fprintf(f,'%% Mass matrix for algebraic simulations.\n');
      fprintf(f,'M = [\n');
      for i = 1:size(obj.massMatrix,1)
        fprintf(f,'\t');
        fprintf(f,'%g\t',obj.massMatrix(i,:));
        fprintf(f,'\n');
      end
      fprintf(f,'];\n\n');
    ...
    function [] =  generateDriverOdeFunction(obj,name)
      ...
      fprintf(fm,'[p,x0,M] = %sDefaultParam();\n',class(obj.model));
